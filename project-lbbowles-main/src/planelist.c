...
/*****************************************************************************
* planelist_clearflight is called by the queue_handler_thread and is what 
* checks to see if the id linked to the front of the taxi list is on the planes
* list (it always should be), and then set the planes state to clear  
*/
void planelist_clearflight(char *flight_id) {
    pthread_rwlock_rdlock(&listlock);       //lock 

    for (int i = 0; i < alist_size(&all_planes); i++) {     //for the entire list of planes
        airplane *thisplane = alist_get(&all_planes, i);     //temporary airplane to check the airplane that is currently at i 

        if ((thisplane->state != PLANE_UNREG) &&        //if not unregistered and if the flight ID is the same
            (strcmp(thisplane->id, flight_id) == 0)) {

            if (thisplane->state != PLANE_CLEAR) {      //if the airplane is already not cleared
                printf("Clearing flight %s\n", flight_id);      //print to server
                fprintf(thisplane->fp_send, "TAKEOFF\n");       //print to client
                thisplane->state = PLANE_CLEAR;     //set to clear
            }
        }
    }

    pthread_rwlock_unlock(&listlock);      //unlock
}
